...
import { collection, query, getDocs, limit, where, onSnapshot, startAfter, deleteDoc, doc, orderBy, updateDoc } from "firebase/firestore";
...
export default function Customer() {
    ...
    const fetchData = (value: any) => {
        const results = items.filter((Booking: { b_cusName: string; b_salesTax: string }) => {
            return (
                value &&
                Booking &&
                Booking.b_cusName &&
                Booking.b_cusName.slice(0, 18).includes(value) ||
                Booking.b_cusName.toLowerCase().includes(value) ||
                Booking.b_cusName.toUpperCase().includes(value) ||
                Booking.b_salesTax &&
                Booking.b_salesTax.slice(0, 18).includes(value) ||
                Booking.b_salesTax.toLowerCase().includes(value) ||
                Booking.b_salesTax.toUpperCase().includes(value)
            );
        });
    ...
    const handleDelete = async (id: any) => {
        ...
        const confirmBox = window.confirm(
            `คุณต้องการลบข้อมูล ${t_ID} หรือไม่?`
        )

        if (confirmBox == true) {
            alert("ลบข้อมูลสำเร็จ")
            try {
                const SwapUpdateRef = doc(db, "Truck", t_ID);
                // สถานะ
                            ...
                            currentPosts?.map((u: {
                                ...
                                <tr key={u.id} className="">
                                    ...
                                    <td className="flex lg:gap-6 max-lg:space-x-4 justify-center my-2" >
                                        ...
                                        {role === "admin" ? <button className="flex justify-between rounded-md text-center text-white bg-amber-500 hover:bg-amber-600 p-2 max-w-20 w-full text-sm" onClick={() => dbEdit(u.b_IDtruck)}  ><Edit_icon color="#fff" /> แก้ไข</button> : ""}
                                        {role === "admin" ? <button className="flex justify-between rounded-md text-center text-white bg-red-500 hover:bg-red-600 max-xl:hidden p-2 max-w-20 w-full text-sm" onClick={() => handleDelete([u.b_IDtruck, u.id])} ><Delete_icon color="#fff" />ลบ</button> : ""}
                                    </td>
                                </tr>
