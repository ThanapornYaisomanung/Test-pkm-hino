...
export default function Garage() {
    ...
    const fetchData = (value: any) => {
        const results = items.filter((Garage: { garage_name: string; }) => {
            return (
                value &&
                Garage &&
                Garage.garage_name &&
                Garage.garage_name.slice(0, 8).includes(value) ||
                Garage.garage_name.toLowerCase().includes(value) ||
                Garage.garage_name.toUpperCase().includes(value)
    ...
    const dbView = async (id: string) => {
        console.log(id);
        router.push(`/database/garage/viewGarage/${id}`)

    }

    const dbEdit = async (id: string) => {
        console.log(id);
        router.push(`/database/garage/editGarage/${id}`)
    ...
    return (
        <div className="md:pt-8 md:pl-8 p-4 max-w-[1680px] w-full min-h-screen ">
            ...
            <div className=" py-6 flex flex-row-reverse">
                {
                    role === "admin" ? <Link className="w-24 bg-green-500 text-white rounded-md hover:bg-green-600 h-8 text-center py-1" href='/database/garage/createGarage'>เพิ่มข้อมูล</Link>
                        : ""
                }
            </div>
                ...
                <table className="table-fixed  max-w-[1680px] w-full">
                    <thead >
                        <tr >
                            ...
                            <th className="">ชื่ออู่</th>
                            <th className=" max-md:hidden">ที่อยู่</th>
                        ...
                        {value.length != 0 ?
                            currentPosts?.map((u: {
                                ...
                                <tr key={u.id} className="" >
                                    <td className="pt-2">{u.garage_name}</td>
                                    <td className="text-center pt-2 max-md:hidden">{u.garage_address}</td>
                                    <td className="text-center pt-2 max-md:hidden">{u.garage_tel}</td>
