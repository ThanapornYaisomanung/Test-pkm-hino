...
export default function TruckType() {
    ...
    const fetchData = (value: any) => {
        const results = items.filter((TruckType: { type_t_name: string; }) => {
            return (
                value &&
                TruckType &&
                TruckType.type_t_name &&
                TruckType.type_t_name.slice(0, 8).includes(value) ||
                TruckType.type_t_name.toLowerCase().includes(value) ||
                TruckType.type_t_name.toUpperCase().includes(value)
    ...
    const dbView = async (id: string) => {
        console.log(id);
        router.push(`/database/trucktype/viewTruckType/${id}`)

    }

    const dbEdit = async (id: string) => {
        console.log(id);
        router.push(`/database/trucktype/edittrucktype/${id}`)
    ...
    return (
        <div className="md:pt-8 md:pl-8 p-4 max-w-[1680px] w-full min-h-screen ">
            ...
            <div className=" py-6 flex flex-row-reverse">
                {
                    role === "admin" ? <Link className="w-24 bg-green-500 text-white rounded-md hover:bg-green-600 h-8 text-center py-1" href='/database/trucktype/createTruckType'>เพิ่มข้อมูล</Link>
                        : ""
                }
            </div>
                ...
                <table className="table-fixed  max-w-[1680px] w-full">
                    <thead >
                        <tr >
                            ...
                            <th className="">ชื่อประเภทรถ</th>
                            <th className=" max-md:hidden">Code ประเภทรถ</th>
                        ...
                        {value.length != 0 ?
                            currentPosts?.map((u: {
                                ...
                                <tr key={u.id} className="" >
                                    <td className="pt-2">{u.type_t_name}</td>
                                    <td className="text-center pt-2 max-md:hidden">{u.type_t_code}</td>
