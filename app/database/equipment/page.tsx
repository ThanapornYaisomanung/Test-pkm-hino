...
export default function Equipment() {
    ...
    const fetchData = (value: any) => {
        const results = items.filter((Equipment: { et_name: string; }) => {
            return (
                value &&
                Equipment &&
                Equipment.et_name &&
                Equipment.et_name.slice(0, 8).includes(value) ||
                Equipment.et_name.toLowerCase().includes(value) ||
                Equipment.et_name.toUpperCase().includes(value)
    ...
    const dbView = async (id: string) => {
        console.log(id);
        router.push(`/database/equipment/viewEquipment/${id}`)
    ...
    return (
        <div className="md:pt-8 md:pl-8 p-4 max-w-[1680px] w-full min-h-screen ">
            ...
            <div className=" py-6 flex flex-row-reverse">
                ...
                {
                    role === "admin" ? <Link className="w-24 bg-green-500 text-white rounded-md hover:bg-green-600 h-8 text-center py-1" href='/database/equipment/createEquipment'>เพิ่มข้อมูล</Link>
                        : ""
                        ...
                        {value.length != 0 ?
                            currentPosts?.map((u: {
                                ...
                                <tr key={u.id} className="" >
                                    <td className="pt-2">{u.et_name}</td>
                                    <td className="text-center text-wrap pt-2 max-md:hidden">
                                        <div className="flex gap-2  truncate ">
                                            {u.et_option?.map((o, index) => (
                                                <p key={index} className="">{o.eType}</p>
                                            ))}
                                        </div>
