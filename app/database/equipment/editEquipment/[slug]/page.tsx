...
import { useState, useEffect, SetStateAction, Suspense, ChangeEvent } from "react";
import { collection, query, getDocs, addDoc, where, onSnapshot, doc, getDoc, getFirestore, updateDoc, deleteField, arrayRemove, arrayUnion } from "firebase/firestore";
...
export default function EditEquipment(props: any) {
    ...
    async function functionSave() {
        ...
        if (confirmBox == true) {
            alert("บันทึกสำเร็จ")
            try {
                const SwapUpdateRef = doc(db, "Equipment", DataID);

                // ชื่ออุปกรณ์
                if (etName === "undefined" || etName === "") {
                    await updateDoc(SwapUpdateRef, {
                        et_name: items.et_name,
                    });
                } else {
                    await updateDoc(SwapUpdateRef, {
                        et_name: etName,
                ...
                // option
                if (etOption > []) {
                    for (let i = 0; i < etOption.length; i++) {
                        await updateDoc(SwapUpdateRef, {

                            et_option: arrayUnion(etOption[i])
                            // et_option: arrayUnion({"eType": etOption})
                        });
                        console.log(etOption[i])
                    }
    ...
    const Deletefields = async (d: any) => {
        const listingRef = doc(data, DataID);
        if (d == d) {
            alert("ลบข้อมูลสำเร็จ")
            try {
                await updateDoc(listingRef, {
                    et_option: arrayRemove({ "eType": d })
